...
<script>
    ...
    import { handleLogout } from '$lib/auth.js';
    import {
        sendQuery,
        loadSurveys,
        submitNewSurvey,
...
//variables for creating new question
  ...
  //Clears up the UI variables for sending the mutation while creating a new survey
    ...
    function openQuestionPopup(action, question = {}) {
      showQuestionPopup.set(true); //triggers front end change
      currentAction.set(action); //decides whether 'creating new' or 'edit'
      currentOption = ''; //resets the options input
      options.set([]); //resets the option list 
    ...
    function addOption() {
      if (currentOption.trim()) {
        options.update(opts => [...opts, currentOption]);
